...
// Minimal OTLP/HTTP-JSON exporters for the Embrace no-account demo path.
//
// EmbraceIO 6.20.0 pulls in OpenTelemetry-Swift-Sdk (the SpanExporter /
// LogRecordExporter protocols) but NOT an off-the-shelf OTLP HTTP exporter pod
// (OpenTelemetryProtocolExporterHttp is absent). Rather than add gRPC/protobuf
// pods that risk clashing with Embrace's pinned OTel version, we hand-roll a
// tiny exporter that POSTs OTLP/JSON straight to grafana/otel-lgtm. This is the
// thing we hand to Embrace.Options(export:) to prove no-account export works.
//
...
final class OtlpJsonSpanExporter: SpanExporter {
  func export(spans: [SpanData], explicitTimeout: TimeInterval?) -> SpanExporterResultCode {
    ...
    let otlpSpans: [[String: Any]] = spans.map { s in
      // OTLP SpanKind: 0=UNSPECIFIED,1=INTERNAL,2=SERVER,3=CLIENT,... map by name.
      let kindCode: Int
      switch s.kind {
